...
export function __splitDate(str, mask, dateLocale, calendar, defaultModel) {
  ...
  const langOpts = dateLocale !== void 0 ? dateLocale : lang.date;
...
function getChange(date, mod, add) {
  ...
  Object.keys(mod).forEach((key) => {
    ...
    const op = key === "year" ? "FullYear" : capitalize(key === "days" ? "date" : key);
...
export function isBetweenDates(date, from, to, opts = {}) {
  ...
  const d2 = getDateIdentifier(to, opts.onlyDate);
  const cur = getDateIdentifier(date, opts.onlyDate);
...
export function formatDate(val, mask, dateLocale, __forcedYear, __forcedTimezoneOffset) {
  ...
  const locale = dateLocale !== void 0 ? dateLocale : lang.date;
